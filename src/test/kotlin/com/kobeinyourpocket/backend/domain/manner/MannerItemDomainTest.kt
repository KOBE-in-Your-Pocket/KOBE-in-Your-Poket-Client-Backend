...
class MannerItemIdTest {
    @Test
    fun `前後空白は trim される`() {
        assertEquals("no-photo-in-shrine", MannerItemId.of("  no-photo-in-shrine  ").value)
    }

    @Test
    fun `空文字は拒否する`() {
        assertFailsWith<IllegalArgumentException> { MannerItemId.of("   ") }
    }
}
...
class MannerLocalizationsTest {
    private val ja = MannerLocalization(title = "撮影禁止", description = "境内での撮影は控えましょう。")
    private val en = MannerLocalization(title = "No photography", description = "Please refrain from taking photos.")

    @Test
    fun `要求言語のローカライズを返す`() {
        val localizations = MannerLocalizations.of(mapOf(Language.JA to ja, Language.EN to en))

        assertEquals(ja, localizations.resolve(Language.JA))
    }

    @Test
    fun `要求言語が無ければ en へフォールバックする（要件定義 D1）`() {
        val localizations = MannerLocalizations.of(mapOf(Language.JA to ja, Language.EN to en))
    ...
    @Test
    fun `en を含まない場合は拒否する（ja のみでも不可）`() {
        assertFailsWith<IllegalArgumentException> {
            MannerLocalizations.of(mapOf(Language.JA to ja))
        }
    }
}
...
class MannerItemTest {
    @Test
    fun `言語非依存フィールドで MannerItem を生成できる`() {
        val item =
            MannerItem.create(
        ...
        assertEquals(MannerScope.LOCAL, item.scope)
        assertEquals(listOf(RelatedSpotId.of("kobe-port-tower")), item.relatedSpotIds)
    }
    ...
    fun `relatedSpotIds は省略時に空`() {
        val item =
            MannerItem.create(
                id = MannerItemId.of("no-littering"),
