 ...
 * フォールバック先である ja の存在を不変条件として保証する。[Companion.of] が生成入口。
 */
data class SpotLocalizations(
    ...
    /**
     * 要求 [language] のローカライズを返す。無ければ [Language.DEFAULT]（ja）へフォールバックする。
     * ja は `init` の不変条件で常に存在するため、本メソッドは必ず非 null を返す。
     */
